...
class RegistrationForm(FlaskForm):
    ...
    username = StringField(_l(u'Username'), validators=[InputRequired(), Length(1, 64),
                                              Regexp('^[A-Za-z][A-Za-z0-9_.]*$', 0,
                                                     _l(u'Username must have only letters, numbers, dots or underscores.'))])  # 用户名只能包括字母，数字，点和下划线
    password = PasswordField(_l(u'Password'), validators=[InputRequired(), Length(1, 64)])
    confirmed = PasswordField(_l(u'Confirm password'), validators=[InputRequired(), EqualTo('password',
                                                                           _l(u'Passwords must match.'))])  # 两次输入的密码必须相同。
